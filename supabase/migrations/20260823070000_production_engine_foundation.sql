-- FusionLab Production Engine foundation.
-- Generated from packages/durable-execution/sql/001_generation_v2_durability.sql
-- after local transaction, idempotency, ledger, outbox, recovery, and audit tests.
-- Do not edit this migration independently; update the canonical schema first.
...
-- Production exposure boundary: fusion_engine is server-only and must never be
-- reachable by browser database roles, even if the schema is accidentally
-- added to the Data API's exposed schemas later.
REVOKE ALL ON SCHEMA fusion_engine FROM PUBLIC, anon, authenticated;
REVOKE ALL ON ALL TABLES IN SCHEMA fusion_engine FROM PUBLIC, anon, authenticated;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA fusion_engine FROM PUBLIC, anon, authenticated;
REVOKE ALL ON ALL FUNCTIONS IN SCHEMA fusion_engine FROM PUBLIC, anon, authenticated;
ALTER DEFAULT PRIVILEGES IN SCHEMA fusion_engine REVOKE ALL ON TABLES FROM PUBLIC, anon, authenticated;
ALTER DEFAULT PRIVILEGES IN SCHEMA fusion_engine REVOKE ALL ON SEQUENCES FROM PUBLIC, anon, authenticated;
ALTER DEFAULT PRIVILEGES IN SCHEMA fusion_engine REVOKE ALL ON FUNCTIONS FROM PUBLIC, anon, authenticated;

ALTER TABLE fusion_engine.creative_projects ENABLE ROW LEVEL SECURITY;
ALTER TABLE fusion_engine.admin_control_plane_state ENABLE ROW LEVEL SECURITY;
ALTER TABLE fusion_engine.asset_access_events ENABLE ROW LEVEL SECURITY;
